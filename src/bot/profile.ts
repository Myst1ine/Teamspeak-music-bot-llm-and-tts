...
export class BotProfileManager {
  ...
  setCustomAvatar(buffer: Buffer | null): void {
    ...
    // During startup, BotInstance may load custom avatar before TS connects.
    // In that phase file-transfer calls would throw "Not connected" and
    // produce noisy warnings. onConnect() will apply the idle avatar.
    if (this.tsClient.getClientId() <= 0) return;
