...
import { AiChatAssistant } from "../ai/chat.assistant.js";
import { HttpTtsProvider } from "../ai/tts.http.js";
...
export class BotInstance extends EventEmitter {
  ...
  private aiAssistant: AiChatAssistant;
  private ttsProvider: HttpTtsProvider;
  private lastSayAt = 0;
  private isAiTtsActive = false;
  private static readonly SAY_COOLDOWN_MS = 5000;
  private static readonly SAY_MAX_CHARS = 120;
  private static readonly AI_TTS_VOICE = "cute";
  private static readonly AI_DUCK_RATIO = 0.35;
  private static readonly AI_DUCK_FADE_MS = 280;
  private static readonly AI_TTS_VOLUME_BOOST = 30;
  private static readonly AI_MIN_TTS_VOLUME = 100;
  ...
  constructor(options: BotInstanceOptions) {
    ...
    this.aiAssistant = new AiChatAssistant(this.logger, {
      enabled: this.config.aiEnabled,
      apiKey: this.config.aiApiKey,
      baseUrl: this.config.aiBaseUrl,
      model: this.config.aiModel,
    });
    this.ttsProvider = new HttpTtsProvider(this.logger, {
  ...
  private async cmdAi(cmd: ParsedCommand): Promise<string> {
    if (!cmd.args) return "Usage: !ai <your request>";
    const now = this.queue.current();
    const action = await this.aiAssistant.run(cmd.args, {
    ...
    if (action.type === "reply") {
      const text = action.text || "No response";
      const ttsInfo = await this.speakAiReply(text);
      return ttsInfo ? `${text}\n(${ttsInfo})` : text;
    }
    ...
    if (action.platform === "qq") flags.add("q");
    else if (action.platform === "bilibili") flags.add("b");
    else if (action.platform === "youtube") flags.add("y");

    const playCmd: ParsedCommand = {
      name: "play",
      args: action.query,
      rawArgs: action.query.split(/\s+/).filter(Boolean),
      flags,
    };
    return this.cmdPlay(playCmd);
  }
  ...
  private cmdHelp(): string {
    ...
    return [
      ...
      `${p}ai <request>  — AI chat and smart play`,
      `${p}say <text>    — Speak text in voice channel (TTS)`,
      `${p}vote         — Vote to skip`,
  ...
  private async cmdSay(cmd: ParsedCommand): Promise<string> {
    const text = cmd.args.trim();
    if (!text) return "Usage: !say <text>";
    const info = await this.speakText(text, {
      ignoreCooldown: false,
      requireIdle: true,
    });
    return info ?? "Speaking now...";
  }

  private cleanupTtsFileOnFinish(file: string): void {
    const cleanup = async () => {
      try {
      ...
      this.player.off("trackEnd", onTrackEnd);
      this.player.off("error", onError);
    };
    const onTrackEnd = () => void cleanup();
    const onError = () => void cleanup();
    this.player.once("trackEnd", onTrackEnd);
  ...
  private async speakText(
    text: string,
    options: { ignoreCooldown: boolean; requireIdle: boolean },
  ): Promise<string | null> {
    if (!this.connected) return "TTS skipped: bot is not connected";
    if (!this.ttsProvider.isAvailable()) return "TTS skipped: not configured";
    if (text.length > BotInstance.SAY_MAX_CHARS) {
      return `TTS skipped: text too long (max ${BotInstance.SAY_MAX_CHARS})`;
    }

    if (!options.ignoreCooldown) {
      const now = Date.now();
      const wait = BotInstance.SAY_COOLDOWN_MS - (now - this.lastSayAt);
      if (wait > 0) return `Please wait ${Math.ceil(wait / 1000)}s before next say`;
      this.lastSayAt = now;
    } else {
      this.lastSayAt = Date.now();
    }

    const originalVolume = this.player.getVolume();
    const ttsVolume = Math.min(
      100,
      Math.max(originalVolume + BotInstance.AI_TTS_VOLUME_BOOST, BotInstance.AI_MIN_TTS_VOLUME),
    );

    if (this.player.getState() !== "idle") {
      if (options.requireIdle) {
        return "Please stop music before using say (MVP limitation)";
      }
      // AI auto-speak should prioritize speaking: interrupt current audio playback.
      this.player.stop();
    }

    this.player.setVolume(ttsVolume);
    const tempFile = await this.ttsProvider.synthesizeToTempFile(text);
    this.player.play(tempFile, 0, 0);
    this.cleanupTtsFileOnFinish(tempFile);
    const restore = () => {
      this.player.setVolume(originalVolume);
      this.player.off("trackEnd", restore);
      this.player.off("error", restore);
    };
    this.player.once("trackEnd", restore);
    this.player.once("error", restore);
    this.emit("stateChange");
    return null;
  }

  private async speakAiReply(text: string): Promise<string | null> {
    if (!this.connected) return "TTS skipped: bot is not connected";
    if (!this.ttsProvider.isAvailable()) return "TTS skipped: not configured";
    const normalized = text.replace(/\s+/g, " ").trim();
    if (!normalized) return "TTS skipped: empty text";

    // AI auto-speak should prioritize speaking: interrupt current audio playback.
    this.isAiTtsActive = true;
    this.lastSayAt = Date.now();
    let started = false;
    try {
      // Keep music playing while TTS is being prepared.
      const jpSpeech = await this.aiAssistant.toJapaneseSpeechText(normalized);
      const tempFile = await this.ttsProvider.synthesizeToTempFile(
        jpSpeech,
        BotInstance.AI_TTS_VOICE,
      );

      const wasPlayingMusic = this.player.getState() === "playing" && !!this.queue.current();
      const originalVolume = this.player.getVolume();
      const currentSong = this.queue.current();
      const elapsedBeforeSpeech = this.player.getElapsed();

      if (wasPlayingMusic) {
        const duckVolume = Math.max(10, Math.round(originalVolume * BotInstance.AI_DUCK_RATIO));
        await this.fadeVolume(originalVolume, duckVolume, BotInstance.AI_DUCK_FADE_MS);
      }

      // Single-track pipeline limitation: TTS playback needs to take over the player.
      if (this.player.getState() !== "idle") this.player.stop();
      ...
      const resumeMusic = async () => {
        try {
          if (!wasPlayingMusic || !currentSong) {
            this.player.setVolume(originalVolume);
            this.emit("stateChange");
            return;
          }
          const stillCurrent = this.queue.current();
          if (!stillCurrent || stillCurrent.id !== currentSong.id) {
            this.player.setVolume(originalVolume);
            this.emit("stateChange");
            return;
          }
          let url: string | null = currentSong.url ?? null;
          if (!url) {
            const provider = this.getProviderFor(currentSong.platform);
            url = await provider.getSongUrl(currentSong.id);
          }
          if (!url) {
            this.player.setVolume(originalVolume);
            this.emit("stateChange");
          ...
          this.player.setVolume(Math.max(10, Math.round(originalVolume * BotInstance.AI_DUCK_RATIO)));
          this.player.play(url, elapsedBeforeSpeech, currentSong.duration);
          await this.fadeVolume(
            Math.max(10, Math.round(originalVolume * BotInstance.AI_DUCK_RATIO)),
            originalVolume,
            BotInstance.AI_DUCK_FADE_MS,
          );
          this.emit("stateChange");
        } catch (err) {
          this.logger.warn({ err }, "Failed to resume music after AI TTS");
          this.player.setVolume(originalVolume);
          this.emit("stateChange");
        }
      };
      ...
      const resumeOnce = () => {
        this.player.off("trackEnd", resumeOnce);
        this.player.off("error", resumeOnce);
        void resumeMusic();
      };
      this.player.once("trackEnd", resumeOnce);
      this.player.once("error", resumeOnce);
      return null;
    } finally {
      if (!started) {
        this.isAiTtsActive = false;
      } else {
        const release = () => {
          this.isAiTtsActive = false;
          this.player.off("trackEnd", release);
          this.player.off("error", release);
        };
        this.player.once("trackEnd", release);
        this.player.once("error", release);
      }
    }
  }
